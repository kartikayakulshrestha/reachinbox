...
const dispatch = useDispatch()
  return (
    <div className={`${darkview ? `bg-[#343A40] border-slate-500` : "bg-white border-slate-500"} border border-t-0 border-r-0 flex text-sm top-0 right-0 h-[80px] items-center fixed w-[95%]`}>
    <div className={`text-xl ml-6 font-medium ${darkview ? "text-[#FFFFFF]" : "text-slate-800"}`}>
        Onebox
    </div>
    <div className='ml-auto mr-6 flex'>
        <button className='' onClick={() => { dispatch(viewDark()) }}>
          <div className={`h-6 w-12 rounded-3xl ${darkview?"":"bg-[#DEDEDE] "} border border-slate-500 grid grid-cols-2`}>
            <div className="p-1">
        ...
        <div className={`${darkview?"text-white":"text-black"} mr-3 font-normal text-lg ml-4 flex`}>
          <p>Tim's Workspace </p>
